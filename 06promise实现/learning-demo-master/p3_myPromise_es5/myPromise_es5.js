...
MyPromise.prototype.then = function (onFulfilled, onRejected) {
  ...
  return new MyPromise(function (resFn, rejFn) {
    ...
    function handleFulfilled (value) {
      ...
      //  下面这一段就是重点  then的返回值  被当成下一个promise的resolve的参数  传递下去
...
// 创建promise实例  并调用reject 
MyPromise.reject = function (arg) {
  return new MyPromise(function (resolve, reject) {
    reject(arg)
  })
}
// 所有promise的状态都改变后   再返回一个新的promise  拿到所有promise的resolve出来的值  当作自己的resolve的参数
...
// 多个promise的时候只要有一个先返回状态就可以了   新的promise就拿到resolve出来的值
